...
variable "ssh_host" {
  description = "Shared Postgres EC2 public DNS or IP (aws-shared postgres_hostname / public_ip)"
  type        = string
}

variable "ssh_user" {
  description = "SSH username on the shared EC2"
  type        = string
  default     = "ec2-user"
}

variable "ssh_private_key_path" {
  description = "Absolute path to shared EC2 SSH private key"
  type        = string
}
