...
output "datagen_public_ip" {
  description = "Self-service datagen VM public IP (ShadowTraffic + Risk API host)"
  value       = local.deploy_datagen_vm ? azurerm_public_ip.datagen[0].ip_address : null
}

output "risk_api_endpoint" {
  description = "Effective Risk Scoring API URL (shared HTTPS Elevate, or BYO VM HTTP)"
  value       = local.effective_risk_api_endpoint != "" ? local.effective_risk_api_endpoint : null
}

output "datagen_ssh_command" {
  description = "SSH to BYO datagen VM"
  value       = local.deploy_datagen_vm ? "ssh -i ${local_file.datagen_ssh_private_key[0].filename} ${local.datagen_ssh_user}@${azurerm_public_ip.datagen[0].ip_address}" : null
}
