...
variable "shared_postgres_ssh_private_key_path" {
  description = "Path to azure-shared SSH private key (for per-attendee lifecycle ShadowTraffic on shared VM)"
  type        = string
  default     = ""
}

variable "shared_postgres_ssh_username" {
  description = "SSH username on shared Postgres VM"
  type        = string
  default     = "azureuser"
}
...
variable "enable_shadowtraffic" {
  description = "Self-service (!use_shared): deploy full ShadowTraffic on the datagen VM"
  type        = bool
  default     = true
}
...
variable "datagen_vm_admin_username" {
  description = "SSH admin username for Azure BYO datagen VM"
  type        = string
  default     = "azureuser"
}

variable "datagen_vm_size" {
  description = "Azure BYO datagen VM size (runs ShadowTraffic + Risk API)"
  type        = string
  default     = "Standard_B2s"
}

variable "allowed_cidr_blocks" {
  description = "CIDRs allowed to SSH (22) and Risk API (8089) on the datagen VM"
  type        = list(string)
  default     = ["0.0.0.0/0"]
}
...
variable "enable_risk_udf" {
  description = "Pre-register Risk CONNECTION + lookup_operational_risk UDF (requires Risk API endpoint + JAR)"
  type        = bool
  default     = true
}

variable "enable_risk_api" {
  description = "Self-service: deploy Risk Scoring API on the datagen VM (:8089 HTTP). Elevate uses azure-shared Container Apps."
  type        = bool
  default     = true
}

variable "risk_api_key" {
  description = "API key for Risk Scoring API / Flink CONNECTION (self-service or fallback)"
  type        = string
  sensitive   = true
...
variable "shared_risk_api_endpoint" {
  description = "Shared Risk Scoring API HTTPS URL from azure-shared (Container Apps)"
  type        = string
  default     = ""
}

variable "shared_risk_api_key" {
  description = "API key for shared Risk Scoring API / Flink CONNECTION"
  type        = string
  sensitive   = true
  default     = "riverpay-workshop-risk"
}
...
variable "risk_udf_jar_path" {
  description = "Path to riverpay-risk-udf JAR for pre-registration"
  type        = string
  default     = "../../udf/riverpay-risk/dist/riverpay-risk-udf-1.0.0.jar"
}

variable "enable_flink_mts" {
  description = "Must remain false for instructor-led Elevate (attendees create MTs)"
  type        = bool
  default     = false

  validation {
    condition     = var.enable_flink_mts == false
    error_message = "Instructor-led azure root must keep enable_flink_mts=false."
  }
}

variable "enable_tableflow_topics" {
  description = "Must remain false for instructor-led Elevate (attendees enable Tableflow)"
  type        = bool
  default     = false

  validation {
    condition     = var.enable_tableflow_topics == false
    error_message = "Instructor-led azure root must keep enable_tableflow_topics=false."
  }
}
