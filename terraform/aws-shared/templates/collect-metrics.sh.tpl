...
ST_CONTAINER="shadowtraffic-riverpay"
...
if [ -n "$PROM_RESPONSE" ]; then
  WRITE_ERRORS=$(echo "$PROM_RESPONSE" | grep -E '^(generator_events_failed_total|datagen_events_failed_total)' | awk '{sum += $2} END {print sum+0}')
  ...
  EVENTS_TOTAL=$(echo "$PROM_RESPONSE" | grep -E '^(generator_events_sent_total|datagen_events_sent_total)' | awk '{sum += $2} END {print sum+0}')
