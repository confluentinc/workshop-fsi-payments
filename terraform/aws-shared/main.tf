...
locals {
  ...
  # Auto-size the Postgres EC2 instance based on account_count when postgres_instance_type is empty.
  # Mirrors azure-shared's auto_vm_size tiering.
  auto_postgres_instance_type = (
    var.account_count <= 30 ? "m5.xlarge" : # 4 vCPU / 16 GB
    "m5.2xlarge"                            # 8 vCPU / 32 GB, recommended for 95 CDC connectors
  )
  effective_postgres_instance_type = var.postgres_instance_type != "" ? var.postgres_instance_type : local.auto_postgres_instance_type

  common_tags = merge(
    {
      Project     = "RiverPay FSI Payments Shared"
...
module "postgres" {
  ...
  instance_type         = local.effective_postgres_instance_type
