...
variable "account_count" {
  description = "Number of workshop accounts (used for Postgres instance auto-sizing). Passed by wsa from the spec."
  type        = number
  default     = 95
}

# ---------------------
# Data generation (unused for RiverPay — ST paths are relative; keep for WSA compat)
# ---------------------

variable "data_dir" {
  description = "Unused for RiverPay aws-shared (ShadowTraffic lives under ../../shadowtraffic). Kept for WSA TF_VAR_data_dir injection."
  type        = string
  default     = ""
}

variable "enable_shadowtraffic" {
  description = "Deploy shared Postgres ShadowTraffic (profiles + FX)"
  type        = bool
  default     = true
}
...
variable "ssh_username" {
  description = "SSH username on the shared Postgres EC2"
  type        = string
  default     = "ec2-user"
}

variable "enable_risk_api" {
  description = "Deploy Risk Scoring API on the shared Postgres EC2"
  type        = bool
  default     = true
}

variable "risk_api_key" {
  description = "API key for Risk Scoring API (Bearer token)"
  type        = string
  sensitive   = true
...
variable "postgres_instance_type" {
  description = "EC2 instance type override. Leave empty to auto-size based on account_count (m5.2xlarge recommended for 95 CDC connectors)."
  type        = string
  default     = ""
}
