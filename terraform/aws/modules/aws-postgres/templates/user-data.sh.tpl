...
-- FX rates (USD cross rates) for Flink temporal join; ShadowTraffic upserts ~5s
CREATE TABLE IF NOT EXISTS riverpay.fx_rates (
    currency_code VARCHAR(3) PRIMARY KEY,
    rate_to_usd DOUBLE PRECISION NOT NULL,
    -- Epoch millis; CDC $rowtime drives Flink FOR SYSTEM_TIME AS OF
    updated_at BIGINT NOT NULL
);

-- Mid-market starting rates (USD per 1 unit of foreign currency, approx)
...
\echo 'Tables: riverpay.customer_profiles, riverpay.fx_rates'
...
Tables:
  ...
  - riverpay.fx_rates (USD + GBP, AUD, CAD, JPY, EUR)
