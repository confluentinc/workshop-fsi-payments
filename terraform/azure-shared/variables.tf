...
variable "data_dir" {
  description = "Optional absolute path for generator configs (WSA). Unused when ShadowTraffic ships from repo."
  type        = string
  default     = ""
}
...
variable "risk_api_key" {
  description = "Bearer API key for the shared Risk Scoring API (Flink REST CONNECTION)"
  type        = string
  sensitive   = true
...
variable "enable_risk_api" {
  description = "Deploy shared Risk Scoring API on Azure Container Apps (public HTTPS)"
  type        = bool
  default     = true
}

variable "enable_shadowtraffic" {
  description = "Deploy ShadowTraffic on the shared Postgres VM (profiles + FX → Postgres)"
  type        = bool
  default     = true
}
