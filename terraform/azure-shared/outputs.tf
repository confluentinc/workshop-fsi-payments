...
# abspath: WSA injects this into per-account terraform/azure/, where a relative
# "./sshkey-....pem" would not resolve (key lives under azure-shared/).
output "private_key_path" {
  description = "Absolute path to the shared SSH private key"
  value       = abspath(local_file.ssh_private_key.filename)
}

# WSA injects shared outputs as TF_VAR_shared_<name>. Per-attendee azure expects
# shared_postgres_ssh_private_key_path for lifecycle ShadowTraffic on this VM.
output "postgres_ssh_private_key_path" {
  description = "WSA alias: absolute path to shared SSH private key (TF_VAR_shared_postgres_ssh_private_key_path)"
  value       = abspath(local_file.ssh_private_key.filename)
}

output "postgres_ssh_username" {
  description = "WSA alias: SSH username on shared Postgres VM"
...
output "risk_api_endpoint" {
  description = "Shared Risk Scoring API HTTPS base URL (Azure Container Apps) for Flink CREATE CONNECTION"
  value       = var.enable_risk_api ? "https://${azurerm_container_app.risk_api[0].ingress[0].fqdn}" : null
}
...
output "risk_api_health_url" {
  description = "Health check URL for the shared Risk Scoring API"
  value       = var.enable_risk_api ? "https://${azurerm_container_app.risk_api[0].ingress[0].fqdn}/health" : null
}

output "acr_login_server" {
  description = "Azure Container Registry login server used for the Risk API image"
  value       = var.enable_risk_api ? azurerm_container_registry.workshop[0].login_server : null
}
