...
resource "confluent_provider_integration" "aws" {
  ...
  # Destroy-time only: dependents (Tableflow topics) are removed first, then this
  # sleep runs before the integration DELETE — Confluent often 409s without it.
  # Use only `self` here (destroy provisioners must not rely on var/config).
  provisioner "local-exec" {
    when    = destroy
    command = "echo 'Draining ${self.id} before provider-integration delete...'; sleep 90"
  }
}
