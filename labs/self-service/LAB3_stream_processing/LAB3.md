...
Terraform self-service usually pre-applies these ALTERs. If `SHOW CREATE TABLE` looks incomplete, run:
...
Apply append + watermark patterns on the four lifecycle topics if needed (see `flink/risk_score.sql` / `flink/fx_conversion.sql`).
