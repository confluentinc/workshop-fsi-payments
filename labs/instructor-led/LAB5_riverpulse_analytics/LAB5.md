...
In a batch world this is a change request: a ticket, a backfill, a new table, and a wait. With Confluent's materialized tables it's far easier — a simple edit to the query that's already running, and Flink evolves the table in place.
...
> [!NOTE]
> The `segment` column appears in Databricks within a minute, but every value is `NULL` at first — payments that completed before the change don't carry one. Give it 2–3 minutes for new payments to flow through the updated statement; those are the rows with a segment.
